...
ModAPI.addAPICallback("GuideAPI", function (api) {
    ...
    PageControllers["TE_TO_RECIPE_PAGE"] = function (params, elements, container, section) {
        ...
        if (params.recipes) {
            for (let key in params.recipes) {
                ...
                elements["slotOutput2_" + key + "_" + section] = {
                    type: "slot",
                    x: xp + 140,
                    y: yp + 35,
                    size: 70,
                ...
                if (recipe.dop && recipe.dop.id) {
                    ...
                    if (chance)
                        elements["to_chance_" + key + "_" + section] = {
                            type: "text",
                            x: xp + 215,
                            y: yp + 55,
                            text: chance + "%",
                            font: {color: android.graphics.Color.BLACK, size: 18}
                        };
                } else {
                    slot_output2.id = 0;
                    slot_output2.data = 0;
    ...
    PageControllers["MAGMA_CRUCIBLE_RECIPE_PAGE"] = function (params, elements, container, section) {
        let yp = 80;
        let xp = section === "left" ? 50 : 550;
        elements["magma_crucible_title_" + section] = {
            type: "text",
            x: xp,
            y: 40,
            text: params.title || "",
        ...
        if (params.recipes) {
            for (let key in params.recipes) {
                let recipe = params.recipes[key];
                elements["slotInputCrucible_" + key + "_" + section] = {
                    type: "slot",
                    x: xp,
                    y: yp,
                    size: 70,
                ...
                elements["magma_crucible_bar_" + key + "_" + section] = {
                    type: "image",
                    x: xp + 70,
                    y: yp + 10,
                    bitmap: params.bar_texture || "bars.machine.def_empty",
                    scale: 3.2
                };
                elements["magma_crucible_text_" + key + "_" + section] = {
                    type: "text",
                    text: recipe.fluid + "(" + (recipe.fluidAmount * 1000) + "mb)",
                    x: xp + 150,
                    y: yp + 20,
                    font: {color: android.graphics.Color.BLACK}
                };
                yp += 80;
            }
        }
