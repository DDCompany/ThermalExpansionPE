sawmillManager.add({
    input: {id: 17, data: 0},
    result: {id: 5, data: 0, count: 6},
    second: {id: ItemID.dustSaw, data: 0, count: 1}
});

sawmillManager.add({
    input: {id: 17, data: 1},
    result: {id: 5, data: 1, count: 6},
    second: {id: ItemID.dustSaw, data: 0, count: 1}
});
sawmillManager.add({
    input: {id: 17, data: 2},
    result: {id: 5, data: 2, count: 6},
    second: {id: ItemID.dustSaw, data: 0, count: 1}
});

sawmillManager.add({
    input: {id: 17, data: 3},
    result: {id: 5, data: 3, count: 6},
    second: {id: ItemID.dustSaw, data: 0, count: 1}
});
sawmillManager.add({
    input: {id: 162, data: 0},
    result: {id: 5, data: 4, count: 6},
    second: {id: ItemID.dustSaw, data: 0, count: 1}
});
sawmillManager.add({
    input: {id: 162, data: 1},
    result: {id: 5, data: 5, count: 6},
    second: {id: ItemID.dustSaw, data: 0, count: 1}
});

sawmillManager.add({
    input: {id: 25, data: 0},
    result: {id: 5, data: 0, count: 8},
    second: {id: 331, data: 0, count: 1, chance: 0.25}
});
sawmillManager.add({
    input: {id: 355, data: 0},
    result: {id: 5, data: 0, count: 2},
    second: {id: 35, data: 0, count: 2}
});
sawmillManager.add({
    input: {id: 47, data: 0},
    result: {id: 5, data: 0, count: 3},
    second: {id: 340, data: 0, count: 3, chance: 0.25}
});
sawmillManager.add({
    input: {id: 58, data: 0},
    result: {id: 5, data: 0, count: 2},
    second: {id: ItemID.dustSaw, data: 0, count: 1}
});
sawmillManager.add({
    input: {id: 323, data: 0},
    result: {id: 5, data: 0, count: 1},
    second: {id: ItemID.dustSaw, data: 0, count: 1, chance: 0.5}
});
sawmillManager.add({
    input: {id: 324, data: 0},
    result: {id: 5, data: 0, count: 1},
    second: {id: ItemID.dustSaw, data: 0, count: 1, chance: 0.5}
});
sawmillManager.add({
    input: {id: 427, data: 0},
    result: {id: 5, data: 1, count: 1},
    second: {id: ItemID.dustSaw, data: 0, count: 1, chance: 0.5}
});
sawmillManager.add({
    input: {id: 428, data: 0},
    result: {id: 5, data: 2, count: 1},
    second: {id: ItemID.dustSaw, data: 0, count: 1, chance: 0.5}
});
sawmillManager.add({
    input: {id: 429, data: 0},
    result: {id: 5, data: 3, count: 1},
    second: {id: ItemID.dustSaw, data: 0, count: 1, chance: 0.5}
});
sawmillManager.add({
    input: {id: 430, data: 0},
    result: {id: 5, data: 4, count: 1},
    second: {id: ItemID.dustSaw, data: 0, count: 1, chance: 0.5}
});
sawmillManager.add({
    input: {id: 431, data: 0},
    result: {id: 5, data: 5, count: 1},
    second: {id: ItemID.dustSaw, data: 0, count: 1, chance: 0.5}
});

sawmillManager.add({
    input: {id: 72, data: 0},
    result: {id: 5, data: 0, count: 1},
    second: {id: ItemID.dustSaw, data: 0, count: 1, chance: 0.5}
});
sawmillManager.add({
    input: {id: 96, data: 0},
    result: {id: 5, data: 0, count: 1},
    second: {id: ItemID.dustSaw, data: 0, count: 1, chance: 0.75}
});
sawmillManager.add({
    input: {id: 333, data: 0},
    result: {id: 5, data: 0, count: 2},
    second: {id: ItemID.dustSaw, data: 0, count: 1}
});
sawmillManager.add({
    input: {id: 333, data: 1},
    result: {id: 5, data: 1, count: 2},
    second: {id: ItemID.dustSaw, data: 0, count: 1}
});
sawmillManager.add({
    input: {id: 333, data: 2},
    result: {id: 5, data: 2, count: 2},
    second: {id: ItemID.dustSaw, data: 0, count: 1}
});
sawmillManager.add({
    input: {id: 333, data: 3},
    result: {id: 5, data: 3, count: 2},
    second: {id: ItemID.dustSaw, data: 0, count: 1}
});
sawmillManager.add({
    input: {id: 333, data: 4},
    result: {id: 5, data: 4, count: 2},
    second: {id: ItemID.dustSaw, data: 0, count: 1}
});
sawmillManager.add({
    input: {id: 269, data: 0},
    result: {id: 5, data: 0, count: 1},
    second: {id: ItemID.dustSaw, data: 0, count: 1, chance: 0.75}
});
sawmillManager.add({
    input: {id: 268, data: 0},
    result: {id: 5, data: 0, count: 2},
    second: {id: ItemID.dustSaw, data: 0, count: 1, chance: 0.75}
});
sawmillManager.add({
    input: {id: 271, data: 0},
    result: {id: 5, data: 0, count: 1},
    second: {id: ItemID.dustSaw, data: 0, count: 1}
});

sawmillManager.add({
    input: {id: 290, data: 0},
    result: {id: 5, data: 0, count: 1},
    second: {id: ItemID.dustSaw, data: 0, count: 1}
});

sawmillManager.add({
    input: {id: 270, data: 0},
    result: {id: 5, data: 0, count: 1},
    second: {id: ItemID.dustSaw, data: 0, count: 1}
});

sawmillManager.add({
    input: {id: 54, data: 0},
    result: {id: 5, data: 0, count: 4},
    second: {id: ItemID.dustSaw, data: 0, count: 1}
});

sawmillManager.add({
    input: {id: 103, data: 0},
    result: {id: 360, data: 0, count: 9}
});